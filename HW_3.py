...
def naive_bayes(s, comp_df, obama_count, trump_count):
    ...
    o_pred = obama_count / total_freq
    t_pred = trump_count / total_freq
    ...
    for word in sentence:
        if word in comp_df['Word'].values:
            word_count = comp_df.loc[comp_df['Word'] == word]['Freq'].values
            o_word_count = comp_df.loc[comp_df['Word'] == word]['Obama'].values
            t_word_count = comp_df.loc[comp_df['Word'] == word]['Trump'].values
            o_prob *= (o_word_count + 1) / (word_count + len(comp_df)) #Add 1 in case the count is 0
            t_prob *= (t_word_count + 1) / (word_count + len(comp_df))

    # Calculate probabilities for Obama and Trump
    o_prob = math.log(o_prob * o_pred)
...
# 'A vector of a word' is a list of 0's and 1's where
...
def test_knn_values(list_len, subset, og_list, print_graph=True):
